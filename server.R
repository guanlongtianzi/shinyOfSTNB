...
if(getRversion() >= "2.15.1") utils::globalVariables(c('nb_model','pred.nb','gg_pr_Curve','gg_Cumulative_curve','gg_roc_curve','gg_lift_curve'))
...
shinyServer(function(input, output) {
  ...
  output$summary <- renderPrint({
    ...
    train_data <- train_data[which(train_data$yuqi != 'unknow'),]

    train_data$yuqi <- factor(train_data$yuqi,levels=c('ok','fraud'))
    ...
    nb_model <<- SelfTrain(yuqi~.,train_data,learner('naiveBayes',list()),'pred.nb')
  ...
  output$pr_curve <- renderPlot({
    ...
    n1 <- NCOL(train_data)
    preds <- predict(nb_model,test_data[,1:(n1-1)],type = 'raw')
    or <- order(preds[,'fraud'],decreasing = T)
    labels <- test_data[or,15]
    labels <- ifelse(labels=='fraud',1,0)
    predictions <- preds[or,2]
